...
const MAX_SPAWNPOINTS = 2000; // Maximale Anzahl an Spawnpunkten 
const MAX_ROUTE_LENGTH = 10; // Maximale Routenlänge in Kilometern

// UI-Elemente für Export und Schrittanzahl
const exportRouteButton = document.getElementById('exportRoute');
const exportFenceButton = document.getElementById('exportFence');
const stepCountDiv = document.getElementById('stepCount');

// Variablen zum Speichern der GeoJSON-Daten
...
map.on('draw:created', function (e) {
  ...
  if (type === 'polygon') {
    ...
    // Speichere die aktuelle Fence GeoJSON
...
function processFence(polygon) {
  fetchSpawnPointsWithinFence(
    ...
    (points) => {
      ...
      // Überprüfen, ob die Anzahl der Spawnpunkte den Grenzwert überschreitet
      if (points.length > MAX_SPAWNPOINTS) {
        alert(
          `Zu viele Spawnpunkte (${points.length}). Bitte zeichnen Sie eine kleinere Fläche oder grenzen Sie das Gebiet ein. Maximal erlaubte Anzahl an Spawnpunkten ist ${MAX_SPAWNPOINTS}.`
        );
        hideLoading();
        return;
      }

      // Schätzung der Routenlänge
      const estimatedRouteLength = estimateRouteLength(points);

      if (estimatedRouteLength > MAX_ROUTE_LENGTH) {
        alert(
          `Die geschätzte Routenlänge beträgt ${estimatedRouteLength.toFixed(
            2
          )} km und überschreitet den Grenzwert von ${MAX_ROUTE_LENGTH} km. Bitte zeichnen Sie eine kleinere Fläche oder grenzen Sie das Gebiet ein.`
        );
        hideLoading();
        return;
      }
...
function estimateRouteLength(points) {
  // Falls die Anzahl gering ist, ist die Routenlänge minimal
  if (points.length <= 1) {
    return 0;
  }

  // Berechne die konvexe Hülle (convex hull) der Punkte
  const turfPoints = points.map((pt) => turf.point([pt.lon, pt.lat]));
  const fc = turf.featureCollection(turfPoints);
  const hull = turf.convex(fc);

  if (!hull) {
    // Falls die konvexe Hülle nicht berechnet werden kann, schätzen wir anhand der maximalen Entfernung
    let maxDistance = 0;
    ...
    // Geschätzte Route ist ungefähr doppelt so lang wie die maximale Entfernung
    return maxDistance * 2;
  }

  // Berechne den Umfang der konvexen Hülle als Schätzung für die Routenlänge
  const length = turf.length(hull, { units: 'kilometers' });
...
function createRouteWithinFence(selectedPositions) {
  ...
  // Speichere die aktuelle Route GeoJSON
  currentRouteGeoJSON = turf.featureCollection(route);

  // Aktualisiere die Schrittanzahl in der UI
...
// Funktion zum Aktualisieren der Schrittanzahl in der UI
...
// Funktion zum Exportieren von GeoJSON-Dateien
...
// Event-Handler für den Export der Route
exportRouteButton.addEventListener('click', () => {
  if (!currentRouteGeoJSON) {
    alert('Keine Route zum Exportieren vorhanden.');
    return;
  }
  exportGeoJSON(currentRouteGeoJSON, 'route.geojson');
});

// Event-Handler für den Export der Fence
exportFenceButton.addEventListener('click', () => {
  if (!currentFenceGeoJSON) {
    alert('Keine Fence zum Exportieren vorhanden.');
    return;
  }
  exportGeoJSON(currentFenceGeoJSON, 'fence.geojson');
});
